...
# native operating system is possible via the terminal panel, connections can be made to databases, slide 
# presentations and dashboards can be created, R Markdown reports/notebooks can be rendered and file systems
# can be viewed.


###########################################################################
## PART 1: BASE R FUNDAMENTALS AND BASIC STATISTICAL FUNCTIONS (45 min)
###########################################################################
...
# numeric, character, logical and factor data types (atomic vector types). R has table, matrix, dataframe and list data structures.
...
# Note the use of spacing in the R code which is best practice as it enhances readability
...
# logical
i_like_data <- TRUE
i_like_data <- T # If you are clever!
class(i_like_data)
...
# Now that we have seen the atomic vector types used by R, how are they represented in more 
# complex data structures such as matrices, tables, dataframes and lists?

# Create a matrix with the elements of x
sales <- matrix(x,
                ncol = 5,
                byrow = T)
...
# In fact, R can be used as a calculator as well:
6 + 7
2 ^ 4
2 * 5
2 - 1
...
# Matrices are restricted to having just one atomic vector type for any matrix (matrices can be made up of any single vector type)
class(mtcars)
head(mtcars)
?mtcars

# Dataframes can be indexed in the same way as matrices
mtcars[1, 5]
mtcars[1, "mpg"]
mtcars$car <- rownames(mtcars)
head(mtcars)

mtcars["Hornet Sportabout", "hp"]

# Lists are a little complex but really useful when working with big data
# Example: a function which imports 57,000 CSV files into a list and then appends them together
...
# more to it but this should provide a good starting point for your own R learning journey. Practice
# makes perfect! The only way to get good at coding and statistical analysis is to do plenty of coding
# and statistical analysis. A great place to learn is https://www.datacamp.com/

# summary() is an incredibly useful function for inspecting R objects (lists, matrices, dataframes, models, plots, etc.)
summary(mtcars)
summary(my_list)
summary(sales)
