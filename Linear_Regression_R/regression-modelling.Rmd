...
# Climate data
climate_data <- read.csv("/home/swalsh/MIT/climate_change.csv", stringsAsFactors = FALSE)

# Student reading score data
pisa_train <- read.csv("/home/swalsh/MIT/pisa2009train.csv", stringsAsFactors = TRUE)
pisa_test <- read.csv("/home/swalsh/MIT/pisa2009test.csv", stringsAsFactors = TRUE)

# Google Trends flu data
flu_train <- read.csv("/home/swalsh/MIT/FluTrain.csv", stringsAsFactors = FALSE)
flu_test <- read.csv("/home/swalsh/MIT/FluTest.csv", stringsAsFactors = FALSE)
```
...
**Ordinary Least Squares (OLS) linear regression**, first developed by the French mathematician Adrien-Marie Legendre in 1805, is a statistical technique used for the analysis and modelling of a linear relationship between a continuous dependent variable and one or more independent variables (which can be continuous or categorical). Put simply, if the relationship between two variables is approximately linear, then a straight line can be fit to the data in order to model the relationship. Linear regression is used for both prediction and statistical inference. The regression equation (i.e. the equation of a straight line) using one covariate takes the following form
...
where $\beta_{0}$ is now the intercept and $\beta_{1}$ is now the slope of the line or coefficient.
...
```{r Ex1.slope and intercept, eval = FALSE}
...
3. No/little multicollinearity (which affects significance tests/VIFs can assess)
4. Homoscedasticity (constant variance in residuals/errors of fit)
5. Outliers do not negatively impact the regression

We can test these assumptions using residual histograms, Q-Q plots and residual variance plots. If the residuals are normally distributed, then the Q-Q plot should form a relatively straight line (focus on the centre section as the tails usually deviate). If we need to test the normality of a covariate, we can use the *Shapiro-Wilk* test.
...
Upon visual inspection, the relationship is approximately linear, has a negative direction, and looks to be fairly strong. The **strength of a straight line association** between two normally distributed continuous variables can be quantified using Pearson's correlation coefficient
...
If the relationship is non-linear, and linear regression is still desired, a common approach is to transform the response variable in order to coerce the relationship to one that approaches linearity. **It is also good practice to transform a skewed dependent variable to a more normal one as this reduces the impact of any outliers on the residuals**. Note that $$y_i = \beta_{0} + \beta_{1}x_{1} + e_i$$ is not the only two-parameter model for describing the relationship between a dependent variable and a single continuous explanatory variable. Model choice is vital and alternative options include transformations such as natural log, base ten log, exponential, square root, power law, cube root and inverse. The `mpg` and `disp` relationship is already approximately linear but it can be strengthened using a natural log transformation of the dependent variable `mpg`

```{r Ex1.log(mpg) vs disp}
...
Visually, there is not much difference but if we compare Pearson's correlation coefficients we see an improvement. Spearman's rank is also shown for comparison
...
The next step is to fit a linear regression model to the data. We model `log(mpg)` as a function of `disp` 
...
Looking at the model coefficients, we see both the intercept and engine displacement coefficient estimates have highly significant $p$-values. The $t$-value is a measure of a coefficient's "*signal*" strength and is the estimate divided by its corresponding standard error. A large absolute $t$-value corresponds to a low $p$-value and a strong signal with the sign indicating the effect direction.
...
You will have noticed that the output is identical to model 1. Model diagnostics can be carried out in the same manner 
...
Back to model 1. The coefficients 
...
Recall that standard errors are unreliability measures. Increasing the spread of the $x$ values reduces unreliability of the estimated slope. For the intercept, uncertainty declines with increasing sample size and confidence in predictions made with linear regression declines with the square of the distance between the mean value of $x$ and the value of $x$ where the prediction is to be made. Thus, when the origin of the graph is far from the mean value of $x$, the standard error of the intercept will be large, and vice versa.

### Model interpretation

Using the first simple model, the $p$-value for engine displacement is highly significant and $r^2$, a measure of variance explained, is strong at 0.78. The take home message from the output is that for every unit increase in engine displacement there is, on average, a -0.002115 decrease in fuel efficiency (via the log of mpg). Therefore, we infer from the model that fuel efficiency decreases with increasing engine displacement.
...
Sometimes the literal translation of the intercept is nonsensical but we ignore it in most cases. Statistical significance is also not always important (this just gives us the probability that the coefficient estimate is zero). For example, the expected value of `log(mpg)` when engine displacement is zero is 3.45. Obviously this makes no sense as there would be no engine to measure fuel efficiency from if the displacement was zero cubic inches. At the time of writing, the smallest engine displacement award actually goes to the *Tata Nano* (38 cubic inches).

In our simple linear regression model, we have a statistically significant intercept coefficient. What does this mean? It just means that the intercept value is significantly different from zero. However, as mentioned earlier it cannot be taken literally. We just use it as a constant that is added to, or subtracted from, the linear predictor(s).
...
```{r Ex1.multiple linear regression}
# Create a multiple linear regression model
model_2 <- lm(log(mpg) ~ disp + wt + factor(cyl) + gear, 
              data = mtcars)
...
Engine displacement becomes insignificant when vehicle weight, number of cylinders and gears are included as covariates. Multicollinearity is at play here as some of these covariates are strongly correlated

```{r Ex1.corr matrix}
# Create the correlation matrix
cor(mtcars[c(2, 3, 6, 10)])
vif(model_2)
```

**Whenever we want to include more than one covariate in any regression model, we use the literature or business domain to select candidate predictors**. It is good practice to fit univariate regression models to get the individual effect of each covariate before plugging them all into a multiple regression model. Any major changes in parameter estimates usually signal multicollinearity or a convergence issue. **Backwards elimination** is the advised approach for feature selection and is preferred to any type of stepwise or automated approaches which can be [flawed](https://www.stata.com/support/faqs/statistics/stepwise-regression-problems/). Let's remove engine displacement and gear and update the model

```{r Ex1.final model}
# Create a multiple linear regression model omitting disp and gear
# Assess RMSE
final_linear_fit <- lm(log(mpg) ~ factor(cyl) + wt, # cyl is an ordered factor; 4 is ref naturally
                       data = mtcars)
...
All model coefficients are statistically significant and the RSE and adjusted $r^2$ are also better than any of our earlier models. Model diagnostics can be carried out using the `plot()` function once more and VIFs are also calculated

```{r Ex1.diagnostics3}
# Model diagnostics
par(mfrow = c(2, 2))
plot(final_linear_fit)
vif(final_linear_fit) # VIFs
```
...
pred_mdl <- lm(log(mpg) ~ factor(cyl) + wt, 
               data = training)

# Make predictions on test set
mdl_preds <- predict(pred_mdl, 
                     newdata = test)

# Assess model performance using the RMSE 
# Note the conversion back to the original mpg units
...
```{r Ex2.climate model 1}
# Build a model
climate_lm1 <- lm(Temp ~ MEI + CO2 + CH4 + N2O + CFC.11 + CFC.12 + TSI + Aerosols,
                  data = training)
```
...
```{r Ex2.summary model 1}
# Inspect the model
summary(climate_lm1)
```

The model has a global *p*-value < 0.05 so it is statistically significant and the adjusted R-squared is good at 0.74. However, the coefficients for N2O and CFC.11 are negative which does not seem right as they are GHGs known to have a warming effect on the Earth's atmosphere. Collinearity is likely the issue here as these variables are probably correlated with others in the data set.
...
The reduced model has a global *p*-value < 0.05 and the adjusted R-squared is good at 0.72. The parameter estimate for N2O now makes sense as it is positive and in agreement with scientific consensus. VIFs are all around 1 which indicates no multicollinearity issues.
...
1) A linear relationship between covariates and the dependent variable(s) (this is what the model equation assumes);
...
Finally, calculate R-squared for the test set predictions. The model that returns the lowest R-squared on the test data would be chosen for analysis and possible production as it would explain the greatest amount of variance. This model explains 50% of the variance in the test set.
...
The linear regression routine skips over observations with missing data. Let's remove incomplete observations from both the training and test sets.
...
For true discrete categorical binary data **[0, 1]**, R always uses 0 as the reference group. R also uses alphabetical ordering naturally if no reference level is specified. When R imported the data, we specified that strings should be converted to factors. R uses alphabetic ordering to determine the reference value of factors so the reference for `raceeth` is "*American Indian/Alaska Native*". However, "*White*" is the most frequent ethnicity so we will change the reference to that instead.
...
Okay, again the R-squared values are not impressive but this is likely to be due to the fact that this is a hard prediction problem. Let's make predictions on the test set using this model
...
Google Search Queries - Google Trends allows public retrieval of weekly counts for every query 
searched by users around the world. For each location, the counts are normalized by dividing 
the count for each query in a particular week by the total number of online search queries submitted 
in that location during the week. Then, the values are adjusted to be between 0 and 1.

The CSV file FluTrain.csv aggregates these data from 01 January 2004 until 31 December 2011 as follows:
...
Notes:
Week values are character strings
ILI-related weekly visits as a percentage of all visits ranges from 0.5% to 7.6%
Weekly ILI-related search queries as a proportion of all queries ranges from 0.04 to 1.00
...
number of unusually large or small observations from having an undue influence on the sum of 
squared errors of predictive models. In this problem, we will predict the natural log of the 
ILI variable, which can be computed in R using the log() function.

```{r}
# Plot the natural logarithm of ILI versus Queries. What does the plot suggest?
plot(flu_train$Queries,
     log(flu_train$ILI)) # There is a positive linear relationship between log(ILI) and Queries.
```

Which model best describes our estimation problem?
log(ILI) = intercept + (coefficient * Queries), where the coefficient is positive.
...
Notes:
Over 70% of the variance in log(ILI) is explained by the linear regression model
The model is statistically significant

For a single variable linear regression model, there is a direct relationship between the R-squared 
and the correlation between the independent and the dependent variables. What is the relationship we infer 
from our problem? 
...
However, the dependent variable in our model is log(ILI), so pred_test1 contains predictions 
of the log(ILI) value. We are instead interested in obtaining predictions of the actual ILI value. We can 
convert from predictions of log(ILI) to predictions of actual ILI via exponentiation, or the exp() function. 
...
variables. This sort of dataset is called a "time series." Often, statistical models can be improved by 
predicting the current value of the dependent variable using the value of the dependent variable from 
earlier weeks. In our models, this means we will predict the ILI variable in the current week using values 
of the ILI variable from previous weeks.

First, we need to decide the amount of time to lag the observations. Because the ILI variable is reported 
with a 1- or 2-week lag, a decision maker cannot rely on the previous week's ILI value to predict the current 
week's value. Instead, the decision maker will only have data available from 2 or more weeks ago. We will 
build a variable called ILI_lag2 that contains the ILI value from 2 weeks before the current observation.
...
na.pad = TRUE means to add missing values for the first two weeks of our dataset where we cannot compute the data from 2 weeks earlier.

```{r}
flu_train$ILI_lag2 <- stats::lag(zoo(flu_train$ILI), -2, na.pad = TRUE) %>% coredata()
flu_test$ILI_lag2 <- stats::lag(zoo(flu_test$ILI), -2, na.pad = TRUE) %>% coredata()

# PLot the relationship between log(ILI_lag2) and log(ILI)
plot(log(flu_train$ILI_lag2),
     log(flu_train$ILI))
